...
## 开始之前
### 存储设备
* 插件是依赖于外部存储的，请确认路由器上有存储设备，并预留有安装插件的空间。
* 支持内存卡和usb的路由器，插件存储位置默认位于内存卡上，所以经务必插入内存卡再开始下面的教程。
* 如果路由器上只有 usb 接口，那插件存储位置默认位于usb设备上，所以经务必插入内存卡再开始下面的教程。
* 例外，HC5961 HC5962，因为flash够大，所以无需外置存储。
...
### 如何ssh进入路由器
```bash
# 如果你的路由器ip是： 192.168.199.1
# 开启开发者模式默认的端口是1022，也有可能是22，如果1022不行的话，试一下22端口
# 密码就是你的路由器后台密码
ssh root@192.168.199.1 -p 1022
```
![ssh进入路由器后台](images/ssh_hiwifi.png)
